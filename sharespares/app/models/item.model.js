...
VALUES (?, ?, NOW(), ?, ?, member_id)`}
  ...
  q = `update loftu_reservations set item_return_date = NOW() where item_id = ? AND member_id = ? AND start_date = ? AND end_date=? AND item_return_date IS NULL; UPDATE loftu_items SET status = 'AVAILABLE' WHERE item_id = ?` }
...
Item.remove = (id, result) => {
  sql.query("DELETE FROM loftu_items WHERE id = ?", id, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
...
Item.removeAll = result => {
  sql.query("DELETE FROM loftu_items", (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
